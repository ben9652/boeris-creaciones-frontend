...
import {
  ...
  Product,
  ProductRow,
} from '../../../../../core/models/product.entities';
...
export class ProductsListComponent {
  ...
  visibleExistingProducts: ProductRow[] = [];
  ...
  constructor(
    ...
    effect(() => {
      const selectedProduct: Product | null =
        productsCatalogService.selectedProduct();
      if (selectedProduct !== null) {
        let actualProduct: ProductRow | undefined = this.productsMap.get(
          selectedProduct.id
        );
        ...
        if (actualProduct !== undefined) {
          ...
          if (
            !productsCatalogService.nonModified &&
            productsCatalogService.productUpdated
          ) {
        ...
        // Si es indefinido, significa que no se encuentra en la lista
          ...
          if (selectedProduct.id !== 0) {
            this.productsMap.set(
              selectedProduct.id,
              createProductRow(selectedProduct, selectedProduct)
            );
          }
        }
        this.existingProducts = Array.from(this.productsMap.values());
        this.visibleExistingProducts = this.existingProducts;
      }
      ...
      // Si se apretó el botón de cancelar, es decir, si el producto seleccionado es nulo
        ...
        let productRowNoAffected: ProductRow | undefined =
          this.productsMap.get(id);
  ...
  clickOnProduct(product: ProductRow) {
    this.productsCatalogService.selectedNonModifiedProduct =
      product.nonModified;
    this.productsCatalogService.selectedProduct.set(product.modified);
    this.productsCatalogService.nonModified = areProductsEqual(
      product.nonModified,
      product.modified
    );
    if (this.deviceTypeService.isMobile()) {
  ...
  searchProduct() {
    if (this.existingProducts) {
      this.visibleExistingProducts = this.existingProducts.filter((res) =>
        res.modified.name
          ?.toLocaleLowerCase()
